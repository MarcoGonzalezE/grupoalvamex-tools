...
class sale_order(models.Model):
    ...
    @api.multi
    def importar_ventas(self):
        if self:
            action = self.env.ref('grupoalvamex_tools.sh_import_sol_action').read()[0]
            return action

class ImportarLineasVentasWizard(models.TransientModel):
    _name="import.sol.wizard"

    import_type = fields.Selection([
        ('csv','CSV'),
        ('excel','Excel')
        ],default="excel",string="Tipo de Importacion",required=True)
    file = fields.Binary(string="Archivo",required=True)   
    product_by = fields.Selection([
        ('name','Nombre'),
        ('int_ref','Referencia Interna'),
                            ...
                            if row[0] != '': 
                                ...
                                if search_product:
                                    ...
                                    if row[1] in (None, ""):
                                        ...
                                        search_lote = self.env['stock.picking.batch'].search([('name','=',row[1])], limit = 1)
                                        if search_lote:
                                            vals.update({'batch_id' : search_lote.id})
                                        else:
                                            skipped_line_no[str(counter)]= " - Lote no encontrada. "
                                            counter = counter + 1
                                            continue

                                    if row[2] != '':
                                        vals.update({'kilogram_price' : row[2] })

                                    if row[3] != '':
                                        vals.update({'kilograms' : row[3] })

                                    if row[4] != '':
                                        vals.update({'weighing_ticket' : row[4] })

                                    if row[5] in (None, ""):
                                        continue
                                    else:
                                        search_almacen = self.env['stock.warehouse'].search([('name', '=', row[5])], limit = 1)
                                        if search_almacen:
                                            vals.update({'warehouse_id' : search_almacen.id})
                                        else:
                                            skipped_line_no[str(counter)]= " - Almacen no encontrada. "
                                            counter = counter + 1
                                            continue                                    
                                    ...
                                    if row[7] != '':
                                        vals.update({'product_uom_qty' : row[7] })
                                    else:
                                    ...
                                    if row[8] in (None,"") and search_product.uom_id:
                                        ...
                                        search_uom = self.env['product.uom'].search([('name','=',row[8] )], limit = 1) 
                                        if search_uom:
                                            vals.update({'product_uom' : search_uom.id })
                                        else:
                                            skipped_line_no[str(counter)]= " - Unidad de Medidad no encontrada. " 
                                            counter = counter + 1
                                            continue
                            ...
                            if sheet.cell(row,0).value != '': 
                                ...
                                if search_product:
                                    ...
                                    if sheet.cell(row,1).value in (None, ""):
                                        ...
                                        search_lote = self.env['stock.picking.batch'].search([('name','=',sheet.cell(row,1).value)], limit = 1)
                                        if search_lote:
                                            vals.update({'batch_id' : search_lote.id})
                                        else:
                                            skipped_line_no[str(counter)]= " - Lote no encontrada. "
                                    ...
                                    if sheet.cell(row,2).value != '':
                                        vals.update({'kilogram_price' : sheet.cell(row,2).value })

                                    if sheet.cell(row,3).value != '':
                                        vals.update({'kilograms' : sheet.cell(row,3).value })

                                    if sheet.cell(row,4).value != '':
                                        vals.update({'weighing_ticket' : sheet.cell(row,4).value })

                                    if sheet.cell(row,5).value in (None, ""):
                                        continue
                                    else:
                                        search_almacen = self.env['stock.warehouse'].search([('name', '=', sheet.cell(row,5).value)], limit = 1)
                                        if search_almacen:
                                            vals.update({'warehouse_id' : search_almacen.id})
                                        else:
                                            skipped_line_no[str(counter)]= " - Almacen no encontrada. "
                                    ...
                                    if sheet.cell(row,6).value != '':
                                        vals.update({'name' : sheet.cell(row,6).value })
                                    else:
                                        vals.update({'name' : ('['+search_product.default_code+'] '+search_product.name)})
                                        
                                    if sheet.cell(row,7).value != '':
                                        vals.update({'product_uom_qty' : sheet.cell(row,7).value })
                                    else:
                                        vals.update({'product_uom_qty' : 1 })
                                    ...
                                    if sheet.cell(row,8).value in (None,"") and search_product.uom_id:
                                        ...
                                        search_uom = self.env['product.uom'].search([('name','=',sheet.cell(row,8).value )], limit = 1) 
                                        if search_uom:
                                            vals.update({'product_uom' : search_uom.id })
                                        else:
                                            skipped_line_no[str(counter)]= " - Unidad de Medidad no encontrada. " 
                                    ...
                                    if sheet.cell(row,9).value in (None,""):
                                        ...
                                        vals.update({'price_unit' : sheet.cell(row,9).value })
                                        
                                    if sheet.cell(row,10).value.strip() in (None,"") and search_product.taxes_id:
                                        vals.update({'tax_id' : [(6, 0, search_product.taxes_id.ids)]})
                                    else:
                                        taxes_list = []
                                        some_taxes_not_found = False
